                ...
                <!-- modification des informations -->
                    ...
                    <?php
                        ...
                        if(isset($_GET['erreur'])){
                            if($_GET['erreur'] == 0)
                                echo "<center><p style='color:green'>Modifications réalisées</p></center>";
                            else if ($_GET['erreur'] == 1){
                                echo "<center><p style='color:red'>Mot de passe incorrect, modifications non réalisées</p></center>";
                            }
                            else if ($_GET['erreur'] == 2){
                                echo "<center><p style='color:red'>Veuillez entrer votre mot de passe pour réaliser des changements</p></center>";
                            }
                        }
                    ?>
                    <form action="profil.php" method="post">
                    <h3>Modifiez votre Login</h3>
                        <label for="login">login</label>
                        <input type="text" name="login" id="login" value="<?=$login?>" required>
                        <label for="password">Mot de passe</label>
                        <input type="password" name="password" id="password" placeholder="Entrez votre mot de passe" required>
                        <input type="submit" value="Modifier Votre LOGIN">
                    </form>
                                ...
                                if (password_verify($_POST['password1'], $password)) { // ancien mot de passe correct
                                    if (isset($_POST['newpassword']) && $_POST['newpassword'] !== '' && isset($_POST['newpassword2']) && $_POST['newpassword2'] !== ''){
                                        if ($_POST['newpassword'] == $_POST['newpassword2']){ // nouveau mot de passe correct
                                            ...
                                            $requete = $connect->query("UPDATE `utilisateurs` SET `password`='$password' where `login`='$login';");
